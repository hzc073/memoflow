...
#### Scenario: Video note saves video attachment
- **WHEN** a Xiaohongshu video note is captured in full media mode and a selected direct video candidate downloads successfully
- **THEN** the system SHALL stage the downloaded video as a memo attachment
- **AND** the system SHALL update the local memo attachment list
- **AND** the system SHALL enqueue the attachment for remote upload when remote sync is allowed

#### Scenario: Image note saves image attachments
- **WHEN** a Xiaohongshu image/article note is captured in full media mode and image attachment seeds are available
- **THEN** the system SHALL stage the downloaded images as memo attachments
- **AND** inline image URLs in memo content SHALL point to local staged files when the content references those images
- **AND** the system SHALL enqueue the image attachments for remote upload when remote sync is allowed

#### Scenario: Parser-level image fallback is used when HTML lacks images
- **WHEN** a Xiaohongshu image/article capture has no usable HTML `<img>` sources but the parser exposes valid image attachment seeds
- **THEN** the system SHALL use those parser-level image seeds for attachment preparation
- **AND** the absence of `contentHtml` image nodes SHALL NOT by itself prevent Xiaohongshu image attachment saving
