...
### Requirement: Abnormal tag cleanup follows active recognition policy
The abnormal tag cleanup operation SHALL recompute local derived memo tags using the active workspace `TagRecognitionPolicy`.
...
- **AND** the user confirms abnormal tag cleanup or policy-switch recompute
...
#### Scenario: Custom cleanup follows resolved options
...
#### Scenario: Cleanup explains active policy
- **WHEN** the abnormal tag cleanup or policy-switch recompute confirmation is shown
...
### Requirement: Policy-switch recompute reuses safe self-repair maintenance
The policy-switch recompute action SHALL use the same safe local maintenance ownership as self-repair instead of embedding database mutation logic in settings UI.

#### Scenario: Settings UI starts recompute through maintenance seam
- **WHEN** the user confirms recompute after changing tag recognition policy
...
#### Scenario: Policy-switch recompute preserves user content
