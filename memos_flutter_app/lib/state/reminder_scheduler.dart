...
class ReminderScheduler {
  ...
  void _logError(
    ...
    _ref.read(logManagerProvider).error(
      'Reminder: $message',
      error: error,
      stackTrace: stackTrace,
      context: context,
  ...
  Future<void> initialize() async {
    ...
    _logInfo('launch_details', context: {'didLaunch': didLaunch, 'hasPayload': hasPayload});
  ...
  Future<void> rescheduleAll({bool force = false}) async {
    ...
    if (!force && _lastRescheduleAt != null) {
      ...
      if (elapsed < const Duration(seconds: 3)) {
        _logInfo('reschedule_skip_recent', context: {'elapsedMs': elapsed.inMilliseconds});
        return;
      }
    }
    ...
    for (final row in rows) {
      ...
      for (final time in reminder.times) {
        ...
        if (dndEnd != null && dndEnd.isAfter(now)) {
          ...
          catchUps.add(DateTime(dndEnd.year, dndEnd.month, dndEnd.day, dndEnd.hour, dndEnd.minute));
          continue;
        }
  ...
  Future<bool> sendTestNotification() async {
    ...
    final id = _notificationId('test', DateTime.now().microsecondsSinceEpoch.toString());
    try {
      await _plugin.show(
        id,
      ...
      _logError('test_immediate_failed', error: e, stackTrace: st, context: {'id': id});
      return false;
    }
  }
  ...
  AndroidNotificationDetails _androidDetails(
    ...
    if (settings.soundMode == ReminderSoundMode.custom && settings.soundUri != null) {
  ...
  Future<bool> _scheduleZoned({
    ...
    try {
      ...
      _logWarn('schedule_exact_failed', error: e, stackTrace: st, context: context);
      await _plugin.zonedSchedule(
        id,
        title,
        body,
  ...
  void _handleNotificationResponse(NotificationResponse response) {
    ...
    _logInfo('tap_response', context: {'id': response.id, 'payloadSize': payload.length});
    unawaited(_handleNotificationTap(payload));
  }
