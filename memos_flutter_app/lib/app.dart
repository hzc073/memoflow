...
class _AppState extends ConsumerState<App> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _prefsLoadedSub = _bootstrapAdapter.listenPreferencesLoaded(
      ...
      (_, __) {
        ...
        _requestStartupHandlingFromState(source: 'prefs_loaded');
      },
    );
    _sessionSub = _bootstrapAdapter.listenSession(ref, (_, __) {
      if (_startupHandled) return;
      _requestStartupHandlingFromState(source: 'session');
    });
    _localLibrarySub = ref.listenManual<LocalLibrary?>(
      currentLocalLibraryProvider,
      (_, __) {
        if (_startupHandled) return;
        _requestStartupHandlingFromState(source: 'local_library');
      },
    );
  ...
  void _requestStartupHandlingFromState({String? source}) {
    try {
      final prefsLoaded = _bootstrapAdapter.readPreferencesLoaded(ref);
      final prefs = _bootstrapAdapter.readPreferences(ref);
      final session = _bootstrapAdapter.readSession(ref);
      final hasAccount = session?.currentAccount != null;
      final hasWorkspace =
          hasAccount || _bootstrapAdapter.readCurrentLocalLibrary(ref) != null;
  ...
  Future<void> _handleStartupActions() async {
    ...
    bool hasWorkspace;
    try {
      prefsLoaded = _bootstrapAdapter.readPreferencesLoaded(ref);
      prefs = _bootstrapAdapter.readPreferences(ref);
      final session = _bootstrapAdapter.readSession(ref);
  ...
  bool _handlePendingShare() {
    ...
    if (!_bootstrapAdapter.readPreferencesLoaded(ref)) return false;
    final prefs = _bootstrapAdapter.readPreferences(ref);
